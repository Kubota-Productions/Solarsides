    ...
    public class EoleShaderGUI : ShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor editor, MaterialProperty[] properties)
        {
            ...
            if (isUnfold)
            {
                ...
                DrawCustomSeparator();
                EditorGUILayout.Separator();
                DrawProperties(RegisteredProperties.propNormalMap);

                // Others Color
                EditorGUILayout.Separator();
                DrawCustomSeparator();
                EditorGUILayout.Separator();
                DrawProperties(RegisteredProperties.propOtherColor);

                // ColorMap
                EditorGUILayout.Separator();

                if (BeginGroupWithKeyword("Color Map Blending", Keywords.UseColormap, true, true))
                {
        ...
        private bool BeginGroupWithKeyword(string groupName, Keywords.KeywordProperty keyword, bool useFold, bool isSubGroup = false)
        {
            ...
            if (isKeywordEnabled)
            {
                if (useFold)
                {
                    ...
                    {
                        // Draw keyword
                        DrawProperty(EditorUtils.GetMaterialProperty(keyword.name, properties));
                        //DrawPropertiesFromGroup(target, groupName, propertyNames);
                        return true;
                    }
                    else
                        return false;
        ...
        private void SetCondiationProperties()
        {
            ...
            // Keyword condition ; property to show/hide
        ...
        /// <summary>
        /// Search for a condition value to display this property, if existing. Return true if there is no condition.
        /// </summary>
...
namespace EoleEditor.ShaderProperties
{
    public struct Keywords
    {
        ...
        /*public static KeywordProperty WindSimpleOnly { get; private set; } = new KeywordProperty("_Wind", "_WIND_SIMPLE");
        public static KeywordProperty WindAdvancedOnly { get; private set; } = new KeywordProperty("_Wind", "_WIND_ADVANCED");*/
    ...
    { 
        ...
        public static void InitializeAllRegisteredPropertyName()
        {
            ...
            AllRegisteredPropertiesWithTooltips.Add("_SimpleWindyYOffset", "Vertex position offset in relative Y (fake a crushing effect).");
